...
```
层间依赖
□ 层间依赖单向（L5→L4→L3→L2→L1），无反向引用 [Script]
□ L1/L2 文件不引用 L3-L5 [Script]
□ Skill 间信息流通过文件系统，禁止直接调用 [Script]
...
跨插件独立性
□ 每个 Plugin 能独立运行，不依赖其他 Plugin [人工]
```
...
□ 各阶段产出有 Schema 定义 [人工]
□ SKILL.md 尺寸符合 skill-writing.md 约束 [Script]
□ 被 2+ Skill 引用的知识提取到 knowledge/ [人工]
□ references/ 文件尺寸符合 skill-writing.md 约束 [Script]
□ 一个 Skill 只负责一个方法论阶段；SKILL.md 顶部引用块声明"本 Skill 阶段 = <阶段名>" [人工+留痕]
□ description 含中英文触发短语，禁止摘要工作流步骤 [人工]
```
