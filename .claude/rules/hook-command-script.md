...
## Preset 开发场景适配（claude-context-templates）

> 本节补充 claude-context-templates 项目特有的入口层模式。

### Command 路由到 init.sh / Skill

- Command 文件（`plugin/commands/init-context.md`）仅做路由触发
...
本项目使用 bash 脚本（`scripts/*.sh`）替代 Python 的 `update-context.py`：
